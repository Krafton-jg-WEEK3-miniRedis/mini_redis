...
- 최영빈: 데모 웹과 MongoDB 비교 시나리오, 발표 흐름 정리
- 이창원: redis-cli 스타일 CLI 구현
- 이호준: TCP 서버, RESP2 파서, 명령 라우터 구현
- 고유진: 해시 테이블 저장소, TTL 정책, 저장소 통계, 동시성 보호 구현

## 회고
- 이창원
  - 이번 작업에서 가장 신경 쓴 부분은 mini_redis를 서버 환경에서 안정적으로 실행하고, 다른 클라이언트가 실제로 접근할 수 있게 만드는 것이었다. 서버 파트를 맡으면서 코드 구현도 중요했지만, 도커 환경과 포트 바인딩, 방화벽 설정처럼 실행 환경을 맞추는 일이 생각보다 훨씬 중요하다는 걸 느꼈다.
    특히 어려웠던 점은 포트가 한 번씩 겹치면서, 현재 실행 중인 서버가 내가 직접 띄운 mini_redis인지 아니면 docker에서 이미 돌고 있던 mini_redis인지 구분하기 어려웠던 부분이다. 같은 6379 포트로 접속이 되더라도 어떤 프로세스가 응답하고 있는지 먼저 확인해야 했고, 이 과정에서 서버 확인, 포트 확인, 컨테이너 확인을 함께 보는 습관이 필요하다는 걸 배웠다.

- 이호준
  - AI를 사용해서 구현하는 부분에서 작은 코드베이스를 만들때는 내가 이 코드를 책임질 수 있겠다는 확신이 있었지만 기능이 점점 추가 되고 코드베이스가 커지면서 내가 관리할수 있는 범위를 넘어섰다는걸 느꼈고 더 큰 코드베이스를 책임질수 있는 역량이 부족하다고 느꼈고 이런 문제를 해결하기 위해 어떤 역량이 필요할까에 대한 고민이 생겼다.
 
- 고유진
  - 가장 신경 쓴 부분은 레디스를 그냥 그대로 구현하는 것이 아니라 우리 상황에 맞춰서 학습용으로 변환하는것.
    공부 목적을 위해 C언어가 아닌 느린 파이썬으로 구현하고, 그러면서 기존 레디스 방식을 공부하며 파이썬으로 레디스를 구현했을 때의 비효율성과 그걸 커스터마이징하는 방법을 배웠다.
    AI를 사용하면서 내가 미처 몰랐던 부분들을 구현하니까 이 기능이 정말 필요했던 건지 검토하는 과정이 시간에 쫓겨 나중에 할 수밖에 없었다. 계획을 철저히 했어야하는데 모르는 부분까지 명확하지 않게 계획을 하니까 내 통제에서 벗어난 기분이 들었다.

- 최영빈
  - Codex를 활용해 개발 속도를 높이려 했지만, Docker 배포는 직접 확인해야 하는 설정이 많고 다른 팀원의 변경사항이나 폴더 구조를 완전히 알고 있어야 해서 생각보다 어렵게 느껴졌다. 실제로 캐싱 효과를 보여줄 수 있는 구성을 codex로 구현하는 것이 생각보다 어려웠다.
